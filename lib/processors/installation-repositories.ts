...
import { scheduleRepository, unscheduleRepository } from "@/lib/processors/scheduling.ts";
...
export async function processAddInstallationRepositories({
  ...
  for (const { id, name, full_name } of repositories_added) {
    try {
      if (suspended_at) {
        app.log.info(`ℹ️ Suspended installation. Skipping add repository ${installationId}:${id}:${full_name}`);
        continue;
      }
      ...
      app.log.error(err, `❌ Failed to save or schedule repository ${installationId}:${id}:${full_name}`);
      throw err;
    }
  }
}
...
export async function processRemoveInstallationRepositories({
  ...
  for (const repo of repositories_removed) {
    try {
      app.log.info(`➖ Remove repository ${installationId}:${repo.id}:${repo.full_name}`);
      ...
      app.log.error(err, `❌ Failed to delete or unschedule repository ${installationId}:${repo.id}:${repo.full_name}`);
      throw err;
    }
  }
}
